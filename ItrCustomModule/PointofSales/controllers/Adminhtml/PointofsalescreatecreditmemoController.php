...
class ItrCustomModule_PointOFSales_Adminhtml_PointofsalescreatecreditmemoController extends Mage_Adminhtml_Controller_Action

{
	...
	//*********************************Method for create pdf in magento*********************************//
	  ...
	  public function canCreditmemoAction()
...

	{
...

		{
			...
			$select = $connection->select()->from($pos_sales_report, array('*'))->where('order_id=?',$order_id);// where id =1
			...
			<tr title="#" class="pointer '.$tr_class.'" onclick="showpopup(7,'.$order_id.')">
