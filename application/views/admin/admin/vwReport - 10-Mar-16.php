...
?>
 ...
 <option value="7" >All Party</option>
 
 <!--<option value="5" >Stock In Rent(outword)</option>
...
$newDate = date("d-M-Y", strtotime($originalDate));
		 ...
		 echo $newDate;?></td>
          ...
          <td><?php if($salegs['opc_ppc']==0){ echo "OPC";} else { echo "PPC";}?></td>
...
$newDate = date("d-M-Y", strtotime($originalDate));
      ...
      <td><?php if($purchasegs['opc_ppc']==0){ echo "OPC";} else { echo "PPC";}?></td>
      ...
      <td>-</td>
			 <?php }
		 	 ?>
         </thead><?php  if(empty($saleg)&&empty($outwardg)&&empty($rentg)&&empty($purchaseg)){echo "<tr><td></td><td></td><td>No record found</td><td></td><td></td><td></td></tr>";}else{   ?>
...
$newDate = date("d-M-Y", strtotime($originalDate));
      ...
      <td>-</td>
      <td>-</td>
      <td><?php echo $rows['r_price']; ?></td></tr>
      <?php } ?>
      <?php 
...
$newDate = date("d-M-Y", strtotime($originalDate));
     ...
     <!-- <td><?php echo $stockpurchases['price'];?></td>
      ...
      <?php } if(empty($stockinrent)&&empty($rent)&&empty($sale)&&empty($stockpurchase)){echo "<tr><td></td><td></td><td>No records found</td><td></td><td></td><td></td></tr>";}else {?>
      <tr><td>Total:</td><td></td><td><?php echo $sum;  $sum=$sum/20;echo "&nbsp&nbsp(".$sum." metricton)"; ?></td><td></td><td></td><td></td></tr>
      <?php }?></table>
...
<?php $sum=0;
 ...
 <?php //print_r($inword);die; 
	  foreach($inword as $inwords){
		  ...
		  echo $inwords['noofbags'];?></td><td><?php echo "-";//echo $inwords['d_name'];?></td>
...
$newDate = date("d-M-Y", strtotime($originalDate));
	  ...
	  //foreach($inword as $inwords){
	   ...
	   ?><tr><td>Total:</td><td></td><td><?php echo $sum_in; $sum_in=$sum_in/20; echo "&nbsp &nbsp(".$sum_in." metricton)"; ?></td><td><?php echo $sum; $sum=$sum/20; echo "&nbsp &nbsp(".$sum." metricton)"; ?></td></tr><?php } ?>
</table>
<?php
